...
class bigint {
  ...
  private:
    bigint(int _sign, vector<int> _a) : sign(_sign), a(std::move(_a)) { unify(); }
...
bigint::bigint(int m) { parse_from_long(m); }
bigint::bigint(int64_t m) { parse_from_long(m); }
void bigint::parse_from_long(int64_t m) {
    sign = (m >= 0) ? 1 : -1;
    m = abs(m);
    int length = 0;
    for (int64_t t = m; t > 0; t /= BASE)
        ++length;
    allocate(length);
    for (int i = 0; i < length; ++i, m /= BASE)
        a[i] = m % BASE;
    unify();
}
...
bigint::bigint(const string &s) { parse_from_string(s.c_str()); }
bigint::bigint(const char *s) { parse_from_string(s); }
void bigint::parse_from_string(const char *s) {
    ...
    int L = 0;
    while (s[p + L] >= '0' && s[p + L] <= '9')
        L++;
    allocate((L + NUM_DIGITS - 1) / NUM_DIGITS);
    for (int i = 0; i < L; ++i)
        a[(L - i - 1) / NUM_DIGITS] = a[(L - i - 1) / NUM_DIGITS] * 10 + (s[p + i] - '0');
    unify();
}
...
bigint multiply_brute_force(const bigint &a, const bigint &b) {
    if (a.length() == 0 || b.length() == 0)
        return 0;
    if (a.length() > b.length())
        return multiply_brute_force(b, a);
...
bigint operator*(const bigint &a, const bigint &b) {
    if (a.length() == 0 || b.length() == 0)
        return 0;
    if (a.length() < 32 || b.length() < 32 || max(a.length(), b.length()) <= 256)
        return multiply_brute_force(a, b);
    ...
    for (int i = 0; i < static_cast<int>(z.size()) - 1; ++i)
        z[i + 1] += z[i] / p10, z[i] %= p10;
    ostringstream sout;
    sout << static_cast<int>(z.back());
    for (int i = static_cast<int>(z.size()) - 2; i >= 0; --i)
        sout << setw(L) << setfill('0') << static_cast<int>(z[i]);
    sout.flush();
    return bigint(sout.str()) * (a.sign * b.sign);
}

bigint operator/(const bigint &a, const bigint &b) { return div_and_mod(a, b).first; }

bigint operator%(const bigint &a, const bigint &b) { return div_and_mod(a, b).second; }
...
bigint operator*(int m, const bigint &a) { return a * m; }
...
bigint operator/(const bigint &a, int m) { return div_and_mod(a, m).first; }
int operator%(const bigint &a, int m) { return div_and_mod(a, m).second; }
bigint &bigint::operator+=(const bigint &m) { return *this = (*this) + m; }
bigint &bigint::operator-=(const bigint &m) { return *this = (*this) - m; }
bigint &bigint::operator*=(const bigint &m) { return *this = (*this) * m; }
bigint &bigint::operator/=(const bigint &m) { return *this = (*this) / m; }
bigint &bigint::operator%=(const bigint &m) { return *this = (*this) % m; }
bigint &bigint::operator*=(int m) { return *this = (*this) * m; }
bigint &bigint::operator/=(int m) { return *this = (*this) / m; }
bigint sqrt(const bigint &m) {
    ...
    if (m.length() == 0)
        return 0;
    if (m.length() == 1)
        return static_cast<int>(sqrt(m.a[0] + 1e-2));
...
void write(const bigint &m) { printf("%s", m.to_string().c_str()); }
